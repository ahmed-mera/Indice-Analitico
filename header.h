...
void inserisciParola(NODO * testa){
	...
	do{
		...
		fprintf(fp," %s ",parola);
		system("cls");
		printf("Parola aggiunta correttamente\nPremi 1 per inserire un'altra parola\nPremi 0 per uscire");
		scanf("%d",&controllo);
		//fflush(stdin);
	}while(controllo == 1);
...
void cancellaDaFile(char parola[]){
    ...
    app = fopen("copia.txt","w"); //file di appoggio
        do{
            ...
            if(strcmp(strupr(parola),strupr(parola_file)) == 0){ //se la parola letta e' uguale a quella da eliminare
                continue;
...
int cancella(NODO * teste){
    ...
    if((app->next == NULL) && (strcmp(strupr(app->parola), parola ) == 0)){
    ...
    while (app->next != NULL){
        ...
        if((strcmp(strupr(app->parola), parola ) == 0)){
            *(teste + h(parola[0])) = app->next;
            return 1;
        }

        //prossima parola == a quella cercata
        if(strcmp(strupr(app->next->parola), parola) == 0){
            prev = app->next;
...
void ricerca(NODO * teste){
    ...
    NODO app = *(teste + h(parola[0]));
    while (app){
        if(strcmp(strupr(app->parola), parola ) == 0){ // Parola trovata
